...
export class Scenario implements IScenario {
  ...
  constructor(scenarioDef: IScenarioDefinition) {
    ...
    if (scenarioDef.roleManager) {
      ...
      this.roleManager = new RoleManager(this, scenarioDef.envActors, scenarioDef.roleActors, scenarioDef.roleCommands)
    }
  ...
  addRoleActor(role: Role, configActorForRole: any) {
    ...
    if (typeof configActorForRole.components !== "undefined") {
      configActorForRole.components?.forEach(( compDef: IComponentDefinition) => {
        (actor as Actor3).addComponent(compDef.label, new compDef.component(actor));
      });
    }
    ...
    if (typeof configActorForRole.primary !== "undefined") {
      ...
      setTimeout(() => {
        EventHandler.publish('server:primaryActorAdded', role.getPlayer().getId(), actor.getId())
      },1500)
    }
