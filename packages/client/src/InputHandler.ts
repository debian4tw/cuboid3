...
export class InputHandler {
  ...
  constructor(
    socket: SocketIOClient.Socket | WebWorkerLocalSocketClient,
    document: Document,
    clientScenarioDefs: any
  ) {
    this.sock = socket;
    ...
    EventHandler.subscribe(
      ...
      (actorName: string) => {
        Object.keys(this.clientScenarioDefs).forEach((scenarioName) => {
          if (
            typeof this.clientScenarioDefs[scenarioName].inputs[actorName] !==
            "undefined"
          ) {
            console.log(
              "applying custom inputs from scenarioName",
              scenarioName
            );
            this.clientScenarioDefs[scenarioName].inputs[actorName](
              this.sock,
              this.document
            );
          }
  ...
  defaultAttachInputEvents() {
    ...
    this.document.addEventListener(
      ...
      () => {
        if (
          ...
          this.document.getElementById("game-canvas")
        ) {
    ...
    this.document.onmousemove = (e) => {
      ...
      this.sock.emit("command", {
        label: "mouseMove",
        value: e.movementX * -1 * this.mouseSensitivity,
      });
    };
    ...
    this.document.onkeypress = (e) => {
      ...
      if (statKeys.indexOf(e.key) > -1) {
        ...
        this.sock.emit("scenarioEvent", {
          eventName: "statIncrease",
          data: e.key,
        });
      }
