import { GVector3, GRoundNumber } from '../math/GMath'
import {GParticle} from '../physics/GParticle'
import {GParticleContact} from '../physics/GParticleContact'

describe('1.test over x axis va=2 vb=-3', function() {
  const p1 = new GParticle()
  const p2 = new GParticle()
  ...
  let contact = new GParticleContact(
    [p1,p2],
    1,
    contactNormal
  )
...
describe('4. test elastic over x,y axis: m1=1 p1=(0,0) v1=(1,1) | m2=1 p2=(1,1) v2=(-1,-1)', function() {
  ...
  it('contact normal should be -sqrt(2)/2', function() {
    expect(GRoundNumber(contactNormal.x,10)).toBe(GRoundNumber(-Math.sqrt(2)/2, 10))
  })
  ...
  it('V1 should be -1,1', function() {
    expect(GRoundNumber(p1.getVelocity().x,10)).toBe(GRoundNumber(-1,10))
    expect(GRoundNumber(p1.getVelocity().y,10)).toBe(GRoundNumber(-1,10))
  })

  it('V2 should be 1,1', function() {
    expect(GRoundNumber(p2.getVelocity().x,10)).toBe(GRoundNumber(1, 10))
    expect(GRoundNumber(p2.getVelocity().y,10)).toBe(GRoundNumber(1, 10))
  })
})
...
describe('5. test elastic over x,y axis m1=1kg v1=(1,0) m/s | m2=1kg v2=(0,1) m/s', function() {
  ...
  it('*contact normal should be -1', function() {
    expect(GRoundNumber(contactNormal.x,10)).toBe(-0.7)
  })
  ...
  it('V1 should be ', function() {
    expect(Math.abs(GRoundNumber(p1.getVelocity().x,10))).toBe(0)
  })

  it('V2 should be ', function() {
    expect(GRoundNumber(p2.getVelocity().x,10)).toBe(1)
  })
})
...
describe('6. test elastic over x axis: m1=4kg v1=6m/s | m2=6kg v2=3m/s ', function() {
  ...
  let contact = new GParticleContact([p1,p2], restitutionCoefficient, contactNormal)
