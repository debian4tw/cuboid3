...
/*
export class WSNetworkAdapter implements INetworkAdapter {
  ...
  public onConnect(callback: (socket: WebSocket) => void) {
    this.io.on("open", (socket: WebSocket) => {
      callback(socket);
    });
  }
  ...
  sendToSocketId(
    ...
    this.io.shouldHandle;
    //this.io.sockets.connected[socketId]?.emit(eventName, data);
  }

  joinToRoom(socket: WebSocket, gameId: string) {
    //socket.join(gameId);
  }

  leaveRoom(socket: WebSocket, gameId: string) {
    //socket.leave(gameId);
  }
