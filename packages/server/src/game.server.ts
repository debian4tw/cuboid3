...
export class GameServer {
  ...
  constructor(
    ...
    console.log(
      "Starting cuboid3 game server: version",
  ...
  private attachEvents() {
    ...
    EventHandler.subscribe(
      ...
      (socketId: string, actorId: string) => {
    ...
    EventHandler.subscribe(
      "server:gameCollision",
      (gameId: string, position: GPoint3, label: string) => {
    ...
    EventHandler.subscribe(
      "server:gameEvent",
      (gameId: string, label: string, position: GPoint3, data: any) => {
        // console.log('server:gameCollision', gameId)
        const game = this.findGameById(gameId);
        if (game !== undefined) {
    ...
    EventHandler.subscribe(
      "removeActor",
      (gameId: string, actorLabel: string) => {
        const game = this.findGameById(gameId);
        if (!game) {
          return;
        }
  ...
  createGame(gameId: string) {
    ...
    if (this.gameClassFactory) {
      const game = this.gameClassFactory(
        gameId,
        this.gameDefs,
        this.gameHooksClass
      );
  ...
  onClientConnect(socket: SocketIO.Socket, data: any) {
    ...
    if (joinGame !== undefined) {
      ...
      this.network.sendToSocketId(
        socket.id,
        "scenarioStatus",
        JSON.stringify(joinGame.getScenario()?.getState())
      );
  ...
  attachSocketEvents(socket: SocketIO.Socket) {
    ...
    if ((this.gameHooksClass as any).attachSocketEvents) {
      console.log("attachSocketEvents custom****");
      (this.gameHooksClass as any).attachSocketEvents(socket, this);
    }
    socket.on("playAgain", () => {
    ...
    socket.on("heroSelected", (data) => {
      ...
      this.network.sendToSocketId(
        socket.id,
        "scenarioStatus",
        JSON.stringify(game?.getScenario().getState())
      );
    });
